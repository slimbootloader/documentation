...
Each |RPL| board is assigned with a unique platform ID.
  ...
  +-------------------------+---------------+----------------+---------------+---------------+
  | |RPLS| DDR5 (UDIMM S14) |     0x0019    |      J8G2      |     R6J1      |     rpls      |
  +-------------------------+---------------+----------------+---------------+---------------+
...
For |RPL| platforms, serial port connector location can be found from the above table for each supported target board.
...
To build |SPN| for any |RPL| platform::
...
1. Gather |RPL| IFWI firmware image
  ...
  This image contains additional firmware ingredients that are required boot on |RPL|.
...
Capsule image for |RPL|
^^^^^^^^^^^^^^^^^^^^^^^^^^

|SPN| binary for capsule image could be extracted from |SPN| IFWI image generated in above steps.
...
|SPN| for |RPL| uses BIT16 of PMC I/O register (Over-Clocking WDT Control (OC_WDT_CTL) - Offset 54h) to trigger firmware update. When BIT16 is set, |SPN| will set the boot mode to FLASH_UPDATE.
